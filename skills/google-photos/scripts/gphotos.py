...
def auth_status_from_token(token_path):
    ...
    return {
        ...
        'token_present': bool(getattr(creds, 'token', None))
    }

def list_media_items(creds, page_size=25):
    headers = {
        'Authorization': f'Bearer {creds.token}',
        'Content-Type': 'application/json'
    }
    payload = {'pageSize': page_size}
    response = requests.post('https://photoslibrary.googleapis.com/v1/mediaItems:search', headers=headers, json=payload)
    return {
